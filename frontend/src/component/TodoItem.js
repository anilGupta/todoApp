...
import { Edit, Delete, Archive, Unarchive, AttachFile } from 'styled-icons/material';
import { FilePdf, FileExcel, File, FileWord } from 'styled-icons/fa-regular';
import { baseUrl } from '../constants/Urls';

const TodoItem = ({ id, title, archive, description, handleAction, attachment}) => {
             ...
             <a href={`${baseUrl}/Attachments/files/download/${attachment.name}`}>
               {
                 attachment.type.includes("image")
                   ? <img src={`${baseUrl}/Attachments/files/download/${attachment.name}`} alt={attachment.name} />
                   : attachment.type.includes("excel") ? <FileExcel size="42" title={attachment.name} />
                       : attachment.type.includes("pdf") ? <FilePdf size="42" title={attachment.name} />
                           : attachment.type.includes("word") ? <FileWord size="42" title={attachment.name} />
